...
"""
    process_components_meta(metadata)

Make vector with meta information for components of package, described in `metadata` (typically parsed content of `metadata.json`)
"""
function process_components_meta(metadata)
    result = []
     for (file, props) in metadata
        name =extract_component_name(string(file))
        push!(result, make_component_meta(name, props))
    end
    return result
end

extract_component_name(filename::AbstractString) = split(split(string(filename), "/")[end], ".")[1]
...
function filter_arg(argpair)
    ...
    if haskey(props, :flowType) && haskey(props[:flowType], :name)
        ...
        if arg_type_name == "signature"
            ...
            if !haskey(props[:flowType], :type) || props[:flowType][:type] != "object"
                return false
            end
        end
        return true
